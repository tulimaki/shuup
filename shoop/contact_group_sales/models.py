...
class CustomerGroupSalesRange(models.Model):
    ...
    min_amount = MoneyValueField(verbose_name=_("min amount"), blank=True, null=True)
    max_amount = MoneyValueField(verbose_name=_("max amount"), blank=True, null=True)

    class Meta:
        unique_together = ("group", "shop")

    def clean(self):
        super(CustomerGroupSalesRange, self).clean()
