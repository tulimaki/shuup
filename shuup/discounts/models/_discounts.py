...
def _get_basic_active_query(for_datetime, shop=None):
        query = Q(
            Q(active=True) &
            (Q(start_datetime__isnull=True) | Q(start_datetime__lte=for_datetime)) &
...
class DiscountQueryset(models.QuerySet):
    ...
    def active(self, shop=None):
        return self.filter(_get_basic_active_query(timezone.now(), shop))
    ...
    def archived(self, shop=None):
        ...
        return self.exclude(_get_basic_active_query(timezone.now(), shop)).filter(shop_query)
    ...
    def available(self, shop=None):
        ...
        query = Q(
            _get_basic_active_query(current_local_dt, shop) &
            (
...
class Discount(models.Model, MoneyPropped):
    ...
    shops = models.ManyToManyField(
        "shuup.Shop", blank=True, db_index=True, verbose_name=_("shops"))
