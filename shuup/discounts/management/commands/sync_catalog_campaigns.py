    ...
    def handle(self, *args, **options):
        ...
        for campaign in CatalogCampaign.objects.all():
            ...
            for product in products:
                identifier = "campaign-%s-product-%s" % (campaign.pk, product.pk)
                name = "Campaign %s for product %s" % (campaign.pk, product.name)
                data.update({"product_id": product.pk, "name": name})
                discount, created = Discount.objects.get_or_create(identifier=identifier, defaults=data)
                discount.shops = [campaign.shop]

            for category in categories:
                identifier = "campaign-%s-category-%s" % (campaign.pk, category.pk)
                name = "Campaign %s for category %s" % (campaign.pk, category.name)
                data.update({"category_id": category.pk, "name": name})
                discount, created = Discount.objects.get_or_create(identifier=identifier, defaults=data)
                discount.shops = [campaign.shop]

            for contact in contacts:
                identifier = "campaign-%s-contact-%s" % (campaign.pk, contact.pk)
                name = "Campaign %s for contact %s" % (campaign.pk, contact.name)
                data.update({"contacty_id": contact.pk, "name": name})
                discount, created = Discount.objects.get_or_create(identifier=identifier, defaults=data)
                discount.shops = [campaign.shop]

            for contact_group in contact_groups:
                identifier = "campaign-%s-contact_group-%s" % (campaign.pk, contact_group.pk)
                name = "Campaign %s for contact group %s" % (campaign.pk, contact_group.name)
                data.update({"contact_group_id": contact_group.pk, "name": name})
                discount, created = Discount.objects.get_or_create(identifier=identifier, defaults=data)
                discount.shops = [campaign.shop]

            if not discount:
                continue  # I guess it is possible to have campaign without any sane conditions
