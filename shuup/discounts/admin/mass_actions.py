...
class ArchiveMassAction(PicotableMassAction):
    ...
    def process(self, request, ids):
        ...
        Discount.objects.active(shop).filter(query).update(active=False)
...
class UnarchiveMassAction(PicotableMassAction):
    ...
    def process(self, request, ids):
        ...
        shop = get_shop(request)
        Discount.objects.archived(shop).filter(query).update(active=True)
...
class DeleteMassAction(PicotableMassAction):
    ...
    def process(self, request, ids):
        ...
        shop = get_shop(request)
        Discount.objects.archived(shop).filter(query).delete()
