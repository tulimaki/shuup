...
class ProductMetaBase(ImportMetaBase):
    ...
    post_save_handlers = {
        "handle_variations": ["parent_sku", "variation_value_1", "variation_value_2", "variation_value_3", "variation value 4"],
        "handle_stocks": ["qty"],
        "handle_images": ["image", "media"]
    }

    def handle_variations(self, fields, sess):
    ...
    def handle_stocks(self, fields, sess):
        ...
        if stock_delta != 0:
            ...
            msg = _("Logical stock count set for %(product_name)s set to %(stock_count)s for supplier %(supplier_name)s.") % {
                "product_name": product.name, "stock_count": qty, "supplier_name": supplier.name
            }
            sess.log_messages.append(msg)
