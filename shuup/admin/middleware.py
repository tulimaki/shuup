...
class ShuupAdminMiddleware(object):
    def process_request(self, request):
        ...
        if not is_superuser and Shop.objects.filter(id=request.session.get("admin_shop").id, staff_members__id=request.user.id).exists():
...
if (
    ...
    "shuup.front.middleware.ShuupAdminMiddleware" in settings.MIDDLEWARE_CLASSES
):
